...
if ($action === 'remove_device') {
    ...
    try {
        ...
        echo json_encode(['ok'=>false,'error'=>'Could not remove device: '.$e->getMessage()]);
    }
    exit;
}
...
if ($action === 'delete_user') {
    $uid = (int)($_REQUEST['user_id'] ?? 0);
    if (!$uid) { echo json_encode(['ok'=>false,'error'=>'Missing user_id']); exit; }
    // Cascade: devices → licenses → user group map → user
    try {
        $lic = $pdo->prepare("SELECT id FROM ".TBL_PFX."cellfire_licenses WHERE user_id = ?");
        $lic->execute([$uid]);
        $licRow = $lic->fetch(PDO::FETCH_OBJ);
        if ($licRow) {
            try {
                $pdo->prepare("DELETE FROM ".TBL_PFX."cellfire_devices WHERE license_id = ?")
                    ->execute([$licRow->id]);
            } catch (Exception $e) { /* devices table may not exist */ }
            $pdo->prepare("DELETE FROM ".TBL_PFX."cellfire_licenses WHERE user_id = ?")
                ->execute([$uid]);
        }
        $pdo->prepare("DELETE FROM ".TBL_PFX."user_usergroup_map WHERE user_id = ?")
            ->execute([$uid]);
        $pdo->prepare("DELETE FROM ".TBL_PFX."users WHERE id = ?")
            ->execute([$uid]);
        echo json_encode(['ok'=>true,'message'=>"User $uid deleted"]);
    } catch (Exception $e) {
